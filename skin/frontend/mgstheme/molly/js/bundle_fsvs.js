...
;( function( $, w, d ){
	...
	$.fn.fsvs = function( _options ) {
		...
		var anyActiveFSVS = function() {
			if(fsvsObjects.length > 0){
				for( i = 0; i < fsvsObjects.length; i++ ) {
		...
		var fsvsApp = function( elm ) {
			...
			this.showPagination = function() {
				...
				$outer.remove();
				var $width_scroll = 100 - widthWithScroll;
				$('body.no-slider header > div').css("padding-right", $width_scroll);
			};
			...
			this.hidePagination = function() {
				...
				$('body header > div').removeAttr('style');
			};
